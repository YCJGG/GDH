...
class generator(nn.Module):
    # initializers
    def __init__(self, input_nc, output_nc, ngf=32, nb=9, flag=False):
        super(generator, self).__init__()
        
        if flag ==True:
            self.threshold = 2.44e-5
            self.flag = True
        elif flag == False:
            self.threshold = -1
            self.flag = False
        ...
        self.conv1_norm = nn.BatchNorm2d(ngf)
        self.conv2 = nn.Conv2d(ngf, ngf * 2, 3, 2, 1)
        self.conv2_norm = nn.BatchNorm2d(ngf * 2)
        self.conv3 = nn.Conv2d(ngf * 2, ngf * 4, 3, 2, 1)
        self.conv3_norm = nn.BatchNorm2d(ngf * 4)
        ...
        self.attention1 = nn.Conv2d(ngf*4, 1, 1, 1,0)
        self.attention2 = nn.Softmax()
        self.attention3 = nn.Threshold(self.threshold,0)


        self.deconv1 = nn.ConvTranspose2d(ngf * 4, ngf * 2, 3, 2, 1, 1)
        self.deconv1_norm = nn.BatchNorm2d(ngf * 2)
        self.deconv2 = nn.ConvTranspose2d(ngf * 2, ngf, 3, 2, 1, 1)
        self.deconv2_norm = nn.BatchNorm2d(ngf)
    ...
    def forward(self, input):
        ...
        mask = F.relu(self.attention1(x))
        # B * 1 * H * W
        mask_s = mask.view(mask.size(0),-1)
        mask_s = self.attention2(mask_s)
        mask_s = self.attention3(mask_s)
        mask_s[mask_s>0] = 1
        #print(torch.sum(mask_s))
        mask_ = mask_s[:,np.newaxis,np.newaxis]
        mask = mask_.view(mask.size(0),mask.size(1),mask.size(2),mask.size(3))
        #print(mask)
        #mask = 1 - mask
        if self.flag == True:
            x = x*mask
...
class discriminator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ndf=64):
        ...
        self.conv2_norm = nn.BatchNorm2d(ndf * 2)
        self.conv3 = nn.Conv2d(ndf * 2, ndf * 4, 4, 2, 1)
        self.conv3_norm = nn.BatchNorm2d(ndf * 4)
        self.conv4 = nn.Conv2d(ndf * 4, ndf * 8, 4, 1, 1)
        self.conv4_norm = nn.BatchNorm2d(ndf * 8)
